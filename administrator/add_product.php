...
#$upload_path = $_SERVER['DOCUMENT_ROOT']."/administrator/product_images/";
...
if (!$con){
  ...
  echo"<script language='javascript'> setTimeout(window.location.href='admin_product_add.php',30); </script> ";
...
if (isset($_POST['optionsRadios']) && !empty($_POST['optionsRadios'])) {
    ...
    if (isset($_FILES["file"])) {
        if ((($_FILES["file"]["type"] == "image/gif") || ($_FILES["file"]["type"] == "image/jpeg") || ($_FILES["file"]["type"] == "image/pjpeg") || ($_FILES["file"]["type"] == "image/PJPEG") || ($_FILES["file"]["type"] == "image/jpg") || ($_FILES["file"]["type"] == "image/png") || ($_FILES["file"]["type"] == "image/JPEG") || ($_FILES["file"]["type"] == "image/JPG") || ($_FILES["file"]["type"] == "image/GIF") || ($_FILES["file"]["type"] == "image/PNG")) && ($_FILES["file"]["size"] < 2048000/*限制文件大小小于2m，数值的单位是b*/)){
                            ...
                            if (isset($_POST['textarea_brief']) && !empty($_POST['textarea_brief'])) {
                                if (isset($_POST['textarea']) && !empty($_POST['textarea'])){
                                    if (isset($_POST['name']) && !empty($_POST['name'])){
                                        ...
                                        //------------------------------------------------------------------------------------------------------------------------------------------------------
                                        //将上传文件的信息插入数据库保存
                                        $product_type = $_POST['optionsRadios'];
                                        $product_name = $_POST['name'];
                                        $product_image = $_FILES["file"]["name"];
                                        $product_text_brief = $_POST['textarea_brief'];
                                        $product_text = '';
                                        if (!empty($_POST['textarea'])) {
                                            if (get_magic_quotes_gpc()) {
                                                $product_text = stripslashes($_POST['textarea']);
                                            } else {
                                                $product_text = $_POST['textarea'];
                                            }
                                        }
                                        //-------------------------------！！！！！！！！！！-----------------------------
                                        $product_text = addslashes($product_text);//转义问题，需要和存入数据库时先 addslashes()读出数据库先  htmlspecialchars_decode()
                                        //--------------------------------------------------------------------------------
                                        $del_flag = 0;
                                        ...
                                        //--------------------------------------------------
                                        $res = mysql_query("INSERT INTO product(product_type, product_name, product_image, product_text_brief, product_text, del_flag) VALUE('$product_type', '$product_name', '$product_image', '$product_text_brief', '$product_text', '$del_flag')");  
                                        if ($res){
                                           echo"产品添加成功！";
                                           if ($product_type == 'a') {
            ...
            echo"产品图片应该小于2m，图片类型为jpg、png或gif，请重新处理图片再上传";
            echo"<script language='javascript'> setTimeout(window.location.href='admin_product_add.php',30); </script> ";
        }
    }
    else{

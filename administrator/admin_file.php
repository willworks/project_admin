        ...
        <div class="container-fluid">
            <div class="row">
                <div class="col-sm-3 col-md-2 sidebar">
                    ...
                    <li class="active"><a href="admin_file.php">文件管理</a></li>
                    <li><a href="admin_file_add.php">上传文件</a></li>
                    </ul>
                </div>
                    ...
                    <div class="row">
                        ...
                        while($row = $res->fetch_row()){
                            if ($row[2] == 0) { 
                                ...
                                $name = $row[1];
                                if($row[0] != 'avi' && $row[0] != 'cdr' && $row[0] != 'csv' && $row[0] != 'doc' && $row[0] != 'jpg' && $row[0] != 'mp3' && $row[0] != 'pdf' && $row[0] != 'png' && $row[0] != 'ppt' && $row[0] != 'rar' && $row[0] != 'txt' && $row[0] != 'xls' && $row[0] != 'zip'){
                                    $images = 'unknow';
                                }
                                else{
                                    $images = $row[0];
                        ...
                        */
                        ...
                        if (!$con){
                          ...
                          //--------------------------------------------------
                          $res = mysql_query("SELECT * FROM upload_file");
                          while($row = mysql_fetch_row($res)){
                              if ($row[2] == 0) { 
                              
                                  $name = $row[1];
                                  if($row[0] != 'avi' && $row[0] != 'cdr' && $row[0] != 'csv' && $row[0] != 'doc' && $row[0] != 'docx' && $row[0] != 'jpg' && $row[0] != 'mp3' && $row[0] != 'pdf' && $row[0] != 'png' && $row[0] != 'ppt' && $row[0] != 'pptx' && $row[0] != 'rar' && $row[0] != 'txt' && $row[0] != 'xls' && $row[0] != 'xlsx' && $row[0] != 'zip'){
                                      $images = 'unknow';
                                  }
                                  else{
                                      $images = $row[0];
